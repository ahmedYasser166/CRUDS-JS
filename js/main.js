...
function display() {
  ...
  for (var i = 0; i < booksList.length; i++) {
    cartona += `<tbody class="text-center" > <td class="fw-bold">${i + 1}</td>
      <td class="fw-bold">${booksList[i].code}</td>
      <td class="py-2"><a target="_blank" href="${
        booksList[i].url
      }" > <button id="btnVisit" onclick="visitbook( ${i})" class="btn btn-success text-white btn-submit"> <i class="fa-solid fa-eye"></i>Visit</button></a></td>
      <td class="py-2"><button onclick="deletebook( ${i})" class="btn btn-danger btn-submit"><i class="fa-solid fa-trash"></i> Delete</button>
      </td></tbody>  `;
  }
...
function validatinputs(element) {
  ...
  regex[element.id].test(element.value);
